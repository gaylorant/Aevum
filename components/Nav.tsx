  ...
  return (
    <>
      <header className="sticky top-0 z-40 border-b border-slate-900/80 bg-[#060511]/80 backdrop-blur-xl">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={openModal}
              className="ml-2 rounded-lg px-3 py-2 text-sm font-medium bg-slate-800 text-slate-200 hover:bg-slate-700 transition"
            >
      ...
      {showModal && (
        ...
        >
          <div
            className="bg-[#0e0d1f] border border-slate-800 rounded-2xl shadow-xl w-full max-w-sm p-5"
            onClick={(e) => e.stopPropagation()}
          >
            {!user ? (
              <>
                <h2 className="text-base font-semibold text-slate-100 mb-1">your capsules, saved.</h2>
                <p className="text-sm text-slate-400 mb-4">
                  log in to save Memory Capsules across sessions and access them anytime.
                </p>
                <button
                  onClick={handleLogin}
                  className="w-full flex items-center justify-center gap-2 bg-slate-700 text-slate-100 rounded-xl py-3 text-sm font-medium hover:bg-slate-600 transition mb-3"
                >
              ...
              <>
                ...
                <p className="text-xs text-slate-500 uppercase tracking-wide mb-3">memory capsules</p>
                {loadingCapsules ? (
                  ...
                  <div className="space-y-2 max-h-72 overflow-y-auto">
                    {capsules.map((c) => (
                      <div key={c.id} className="flex items-start justify-between gap-2 bg-slate-800/50 rounded-xl px-3 py-2">
                        <div>
                          <p className="text-sm text-slate-200 leading-snug">{c.content}</p>
                          <p className="text-xs text-slate-500 mt-1">{formatDate(c.created_at)}</p>
                        </div>
                        <button
                          onClick={() => deleteCapsule(c.id)}
                          className="text-slate-600 hover:text-red-400 transition text-xs mt-0.5 shrink-0"
                        >
                          ✕
                        </button>
                      </div>
